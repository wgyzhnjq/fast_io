...
using bignum25519 = std::uint32_t[12];

//using packed32bignum25519 = std::array<packedelem32,5>;

//using packed64bignum25519 = std::array<packedelem64,10>;

inline constexpr packedelem32 bot32bitmask{{0xffffffff, 0x00000000, 0xffffffff, 0x00000000}};
inline constexpr packedelem32 top32bitmask{{0x00000000, 0xffffffff, 0x00000000, 0xffffffff}};
inline constexpr packedelem32 top64bitmask{{0x00000000, 0x00000000, 0xffffffff, 0xffffffff}};
inline constexpr packedelem32 bot64bitmask{{0xffffffff, 0xffffffff, 0x00000000, 0x00000000}};

/* reduction masks */
inline constexpr packedelem64 packedmask26{{0x03ffffff, 0x03ffffff}};
inline constexpr packedelem64 packedmask25{{0x01ffffff, 0x01ffffff}};
inline constexpr packedelem32 packedmask2625{{0x3ffffff,0,0x1ffffff,0}};
inline constexpr packedelem32 packedmask26262626{{0x03ffffff, 0x03ffffff, 0x03ffffff, 0x03ffffff}};
inline constexpr packedelem32 packedmask25252525{{0x01ffffff, 0x01ffffff, 0x01ffffff, 0x01ffffff}};

/* multipliers */
inline constexpr packedelem64 packednineteen{{19, 19}};
inline constexpr packedelem64 packednineteenone{{19, 1}};
inline constexpr packedelem64 packedthirtyeight{{38, 38}};
inline constexpr packedelem64 packed3819{{19*2,19}};
inline constexpr packedelem64 packed9638{{19*4,19*2}};

/* 121666,121665 */
inline constexpr packedelem64 packed121666121665{{121666, 121665}};

/* 2*(2^255 - 19) = 0 mod p */
inline constexpr packedelem32 packed2p0{{0x7ffffda,0x3fffffe,0x7fffffe,0x3fffffe}};
inline constexpr packedelem32 packed2p1{{0x7fffffe,0x3fffffe,0x7fffffe,0x3fffffe}};
inline constexpr packedelem32 packed2p2{{0x7fffffe,0x3fffffe,0x0000000,0x0000000}};

inline constexpr packedelem32 packed32packed2p0{{0x7ffffda,0x7ffffda,0x3fffffe,0x3fffffe}};
inline constexpr packedelem32 packed32packed2p1{{0x7fffffe,0x7fffffe,0x3fffffe,0x3fffffe}};

/* 4*(2^255 - 19) = 0 mod p */
inline constexpr packedelem32 packed4p0{{0xfffffb4,0x7fffffc,0xffffffc,0x7fffffc}};
inline constexpr packedelem32 packed4p1{{0xffffffc,0x7fffffc,0xffffffc,0x7fffffc}};
inline constexpr packedelem32 packed4p2{{0xffffffc,0x7fffffc,0x0000000,0x0000000}};

inline constexpr packedelem32 packed32packed4p0{{0xfffffb4,0xfffffb4,0x7fffffc,0x7fffffc}};
inline constexpr packedelem32 packed32packed4p1{{0xffffffc,0xffffffc,0x7fffffc,0x7fffffc}};

/* out = in */
inline void
curve25519_copy(bignum25519 out, bignum25519 const in) {
	__m128i x0,x1,x2;
	x0 = _mm_load_si128((__m128i*)in + 0);
	x1 = _mm_load_si128((__m128i*)in + 1);
	x2 = _mm_load_si128((__m128i*)in + 2);
	_mm_store_si128((__m128i*)out + 0, x0);
	_mm_store_si128((__m128i*)out + 1, x1);
	_mm_store_si128((__m128i*)out + 2, x2);
}

/* out = a + b */
inline void
curve25519_add(bignum25519 out, bignum25519 const a, bignum25519 const b) {
	__m128i a0,a1,a2,b0,b1,b2;
	a0 = _mm_load_si128((__m128i*)a + 0);
	a1 = _mm_load_si128((__m128i*)a + 1);
	a2 = _mm_load_si128((__m128i*)a + 2);
	...
	_mm_store_si128((__m128i*)out + 0, a0);
	_mm_store_si128((__m128i*)out + 1, a1);
	_mm_store_si128((__m128i*)out + 2, a2);
}

inline void
curve25519_add_reduce(bignum25519 out, bignum25519 const a, bignum25519 const b) {
	__m128i a0,a1,a2,b0,b1,b2;
	__m128i c1,c2,c3;
	...
	_mm_store_si128((__m128i*)out + 0, _mm_unpacklo_epi64(_mm_unpacklo_epi32(r0, r1), _mm_unpacklo_epi32(r2, r3)));
	_mm_store_si128((__m128i*)out + 1, _mm_unpacklo_epi64(_mm_unpackhi_epi32(r0, r1), _mm_unpackhi_epi32(r2, r3)));
	_mm_store_si128((__m128i*)out + 2, _mm_unpackhi_epi32(r4, r5));
}

template<typename... Args>
inline void curve25519_add_after_basic(Args&& ...args)
{
	curve25519_add_reduce(std::forward<Args>(args)...);
}

inline void
curve25519_sub(bignum25519 out, bignum25519 const a, bignum25519 const b) {
	__m128i a0,a1,a2,b0,b1,b2;
	__m128i c1,c2;
	__m128i r0,r1;
...
inline void
curve25519_sub_after_basic(bignum25519 out, bignum25519 const a, bignum25519 const b) {
	__m128i a0,a1,a2,b0,b1,b2;
	__m128i c1,c2,c3;
	__m128i r0,r1,r2,r3,r4,r5;
...
inline void
curve25519_sub_reduce(bignum25519 out, bignum25519 const a, bignum25519 const b) {
	__m128i a0,a1,a2,b0,b1,b2;
	__m128i c1,c2,c3;
	__m128i r0,r1,r2,r3,r4,r5;
...
inline void
curve25519_neg(bignum25519 out, bignum25519 const b) {
	__m128i a0,a1,a2,b0,b1,b2;
	__m128i c1,c2,c3;
	__m128i r0,r1,r2,r3,r4,r5;
...
inline void 
curve25519_mul(bignum25519 out, bignum25519 const r, bignum25519 const s) {
	__m128i m01,m23,m45,m67,m89;
	__m128i m0123,m4567;
	__m128i s0123,s4567;
	__m128i s01,s23,s45,s67,s89;
	__m128i s12,s34,s56,s78,s9;
	__m128i r0,r2,r4,r6,r8;
	__m128i r1,r3,r5,r7,r9;
	__m128i r119,r219,r319,r419,r519,r619,r719,r819,r919;
	__m128i c1,c2,c3;

	s0123 = _mm_load_si128((__m128i*)s + 0);
	...
	s4567 = _mm_load_si128((__m128i*)s + 1);
	...
	s89 = _mm_load_si128((__m128i*)s + 2);
	...
	_mm_store_si128((__m128i*)out + 0, m0123);
	_mm_store_si128((__m128i*)out + 1, m4567);
	_mm_store_si128((__m128i*)out + 2, m89);
}
/*
inline void curve25519_mul_noinline(bignum25519 out, bignum25519 const r, bignum25519 const s) {
	curve25519_mul(out, r, s);
}
*/
inline void curve25519_square_times(bignum25519 r, bignum25519 const in, int count) {
	__m128i m01,m23,m45,m67,m89;
	__m128i r0,r1,r2,r3,r4,r5,r6,r7,r8,r9;
	__m128i r0a,r1a,r2a,r3a,r7a,r9a;
	__m128i r0123,r4567;
	__m128i r01,r23,r45,r67,r6x,r89,r8x;
	__m128i r12,r34,r56,r78,r9x;
	__m128i r5619;
	__m128i c1,c2,c3;

	r0123 = _mm_load_si128((__m128i*)in + 0);
	r01 = _mm_shuffle_epi32(r0123,_MM_SHUFFLE(3,1,2,0));
	r23 = _mm_shuffle_epi32(r0123,_MM_SHUFFLE(3,3,2,2));
	r4567 = _mm_load_si128((__m128i*)in + 1);
	r45 = _mm_shuffle_epi32(r4567,_MM_SHUFFLE(3,1,2,0));
	r67 = _mm_shuffle_epi32(r4567,_MM_SHUFFLE(3,3,2,2));
	r89 = _mm_load_si128((__m128i*)in + 2);
	...
	_mm_store_si128((__m128i*)r + 0, r0123);
	_mm_store_si128((__m128i*)r + 1, r4567);
	_mm_store_si128((__m128i*)r + 2, r89);
}


inline void curve25519_square(bignum25519 r, bignum25519 const in) {
	__m128i m01,m23,m45,m67,m89;
	__m128i r0,r1,r2,r3,r4,r5,r6,r7,r8,r9;
	__m128i r0a,r1a,r2a,r3a,r7a,r9a;
	__m128i r0123,r4567;
...
inline void
curve25519_tangle32(packedelem32 *out, bignum25519 const x, bignum25519 const z) {
	__m128i x0,x1,x2,z0,z1,z2;

	x0 = _mm_load_si128((__m128i *)(x + 0));
	x1 = _mm_load_si128((__m128i *)(x + 4));
	x2 = _mm_load_si128((__m128i *)(x + 8));
	z0 = _mm_load_si128((__m128i *)(z + 0));
	z1 = _mm_load_si128((__m128i *)(z + 4));
	z2 = _mm_load_si128((__m128i *)(z + 8));
...
inline void
curve25519_untangle32(bignum25519 x, bignum25519 z, packedelem32 const *in) {
	__m128i t0,t1,t2,t3,t4,zero;
	...
	_mm_store_si128((__m128i *)x + 0, _mm_unpacklo_epi64(t0, t1));
	_mm_store_si128((__m128i *)x + 1, _mm_unpacklo_epi64(t2, t3));
	_mm_store_si128((__m128i *)x + 2, _mm_unpacklo_epi64(t4, zero));
	_mm_store_si128((__m128i *)z + 0, _mm_unpackhi_epi64(t0, t1));
	_mm_store_si128((__m128i *)z + 1, _mm_unpackhi_epi64(t2, t3));
	_mm_store_si128((__m128i *)z + 2, _mm_unpackhi_epi64(t4, zero));
}

inline void
curve25519_add_reduce_packed32(packedelem32 *out, packedelem32 const *r, packedelem32 const *s) {
...
inline void
curve25519_add_packed32(packedelem32 *out, packedelem32 const *r, packedelem32 const *s) {
...
inline void
curve25519_sub_packed32(packedelem32 *out, packedelem32 const *r, packedelem32 const *s) {
	__m128i r0,r1,r2,r3,r4;
	__m128i s0,s1,s2,s3;
	__m128i c1,c2;
...
inline void
curve25519_sub_after_basic_packed32(packedelem32 *out, packedelem32 const *r, packedelem32 const *s) {
	__m128i r0,r1,r2,r3,r4;
	__m128i s0,s1,s2,s3,s4,s5;
	__m128i c1,c2;
...
inline void
curve25519_tangle64_from32(packedelem64 *a, packedelem64 *b, packedelem32 const *c, packedelem32 const *d) {
	__m128i c0,c1,c2,c3,c4,c5,t;
...
inline void
curve25519_tangle64(packedelem64 *out, bignum25519 const x, bignum25519 const z) {
	__m128i x0,x1,x2,z0,z1,z2,t;

	x0 = _mm_load_si128((__m128i *)x + 0);
	x1 = _mm_load_si128((__m128i *)x + 1);
	x2 = _mm_load_si128((__m128i *)x + 2);
	z0 = _mm_load_si128((__m128i *)z + 0);
	z1 = _mm_load_si128((__m128i *)z + 1);
	z2 = _mm_load_si128((__m128i *)z + 2);
...
inline void
curve25519_tangleone64(packedelem64 *out, bignum25519 const x) {
	__m128i x0,x1,x2;
...
inline void
curve25519_untangle64(bignum25519 x, bignum25519 z, packedelem64 const *in) {
	_mm_store_si128((__m128i *)(x + 0), _mm_unpacklo_epi64(_mm_unpacklo_epi32(in[0].v, in[1].v), _mm_unpacklo_epi32(in[2].v, in[3].v)));
	_mm_store_si128((__m128i *)(x + 4), _mm_unpacklo_epi64(_mm_unpacklo_epi32(in[4].v, in[5].v), _mm_unpacklo_epi32(in[6].v, in[7].v)));
	_mm_store_si128((__m128i *)(x + 8), _mm_unpacklo_epi32(in[8].v, in[9].v)                                                          );
	_mm_store_si128((__m128i *)(z + 0), _mm_unpacklo_epi64(_mm_unpackhi_epi32(in[0].v, in[1].v), _mm_unpackhi_epi32(in[2].v, in[3].v)));
	_mm_store_si128((__m128i *)(z + 4), _mm_unpacklo_epi64(_mm_unpackhi_epi32(in[4].v, in[5].v), _mm_unpackhi_epi32(in[6].v, in[7].v)));
	_mm_store_si128((__m128i *)(z + 8), _mm_unpackhi_epi32(in[8].v, in[9].v)                                                          );
}

inline void
curve25519_mul_packed64(packedelem64 *out, packedelem64 const *r, packedelem64 const *s) {
	__m128i r1,r2,r3,r4,r5,r6,r7,r8,r9;
	__m128i r1_2,r3_2,r5_2,r7_2,r9_2;
	__m128i c1,c2;
...
inline void
curve25519_square_packed64(packedelem64 *out, packedelem64 const *r) {
	__m128i r0,r1,r2,r3;
	__m128i r1_2,r3_2,r4_2,r5_2,r6_2,r7_2;
	__m128i d5,d6,d7,d8,d9;
	__m128i c1,c2;
 ...
 */
inline void
curve25519_contract(unsigned char out[32], bignum25519 const in) {
	alignas(16) bignum25519 f;
	...
	auto carry_pass_full([&]()
	{
		carry_pass();
		f[0] += 19 * (f[9] >> 25); f[9] &= 0x1ffffff;
	});
	auto carry_pass_final([&]()
	{
		carry_pass();
		f[9] &= 0x1ffffff;
	});
...
curve25519_swap_conditional(bignum25519 a, bignum25519 b, uint32_t iswap) {
	uint32_t const swap = (uint32_t)(-(int32_t)iswap);
	__m128i a0,a1,a2,b0,b1,b2,x0,x1,x2;
	__m128i mask = _mm_cvtsi32_si128(swap);
	...
	_mm_store_si128((__m128i *)a + 0, x0);
	_mm_store_si128((__m128i *)a + 1, x1);	
	_mm_store_si128((__m128i *)b + 0, a0);
	_mm_store_si128((__m128i *)b + 1, a1);
	...
	_mm_store_si128((__m128i *)b + 2, a2);
	_mm_store_si128((__m128i *)a + 2, x2);
}

/* out = (flag) ? out : in */
inline void
curve25519_move_conditional_bytes(uint8_t out[96], uint8_t const in[96], uint32_t flag) {
	__m128i a0,a1,a2,a3,a4,a5,b0,b1,b2,b3,b4,b5;
	uint32_t const nb = flag - 1;
	__m128i masknb = _mm_shuffle_epi32(_mm_cvtsi32_si128(nb),0);
	a0 = _mm_load_si128((__m128i *)in + 0);
	a1 = _mm_load_si128((__m128i *)in + 1);
	a2 = _mm_load_si128((__m128i *)in + 2);
	b0 = _mm_load_si128((__m128i *)out + 0);
	b1 = _mm_load_si128((__m128i *)out + 1);
	b2 = _mm_load_si128((__m128i *)out + 2);
	...
	a3 = _mm_load_si128((__m128i *)in + 3);
	a4 = _mm_load_si128((__m128i *)in + 4);
	a5 = _mm_load_si128((__m128i *)in + 5);
	b3 = _mm_load_si128((__m128i *)out + 3);
	b4 = _mm_load_si128((__m128i *)out + 4);
	b5 = _mm_load_si128((__m128i *)out + 5);
	...
	_mm_store_si128((__m128i*)out + 3, a3);
	_mm_store_si128((__m128i*)out + 4, a4);
	_mm_store_si128((__m128i*)out + 5, a5);
}
