...
int main()
{
	...
	__gnu_cxx::stdio_filebuf<char> stdio_filebuf(posix.native_handle(),std::ios::binary|std::ios::out,1048576);
	...
	__gnu_cxx::stdio_filebuf<char> stdio_filebuf(posix.native_handle(),std::ios::binary|std::ios::in,1048576);
	fast_io::streambuf_io_observer fob{std::addressof(stdio_filebuf)};
	for(std::size_t i{};i!=N;++i)
		scan(fob,vec[i]);
	}
